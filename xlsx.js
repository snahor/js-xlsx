...
(function make_xlsx(XLSX) {
  ...
  function insertCommentsIntoSheet(sheetName, sheet, comments) {
    comments.forEach(function(comment) {
      ...
      if (!cell) {
        ...
        var range = utils.safe_decode_range(sheet["!ref"] || "BDWGO1000001:A1");
        var thisCell = utils.decode_cell(comment.ref);
        if (range.s.r > thisCell.r) range.s.r = thisCell.r;
        if (range.e.r < thisCell.r) range.e.r = thisCell.r;
        if (range.s.c > thisCell.c) range.s.c = thisCell.c;
        if (range.e.c < thisCell.c) range.e.c = thisCell.c;
        var encoded = utils.encode_range(range);
  ...
  function parse_comments_xml(data, opts) {
    ...
    (data.match(/<(?:\w+:)?commentList>([^\u2603]*)<\/(?:\w+:)?commentList>/) || ["", ""])[1].split(/<\/\w*:?comment>/).forEach(function(x, index) {
      ...
      var cell = utils.decode_cell(y.ref);
  ...
  function parse_BrtBeginComment(data, length) {
    ...
    out.rfx = rfx.s;
    out.ref = utils.encode_cell(rfx.s);
  ...
  var rc_to_a1 = (function() {
    ...
    function rcfunc($$, $1, $2, $3, $4, $5) {
      ...
      return $1 + utils.encode_col(C) + utils.encode_row(R);
    }
  ...
  function stringify_formula(formula, range, cell, supbooks, opts) {
    ...
    for (var ff = 0, fflen = formula[0].length; ff < fflen; ++ff) {
      ...
      switch (f[0]) {
        ...
        case 'PtgRef':
          type = f[1][0];
          c = shift_cell_xls(utils.decode_cell(utils.encode_cell(f[1][1])), _range);
          stack.push(utils.encode_cell(c));
          break;
          /* 2.5.198.88 */
        case 'PtgRefN':
          type = f[1][0];
          c = shift_cell_xls(utils.decode_cell(utils.encode_cell(f[1][1])), cell);
          stack.push(utils.encode_cell(c));
          break;
        case 'PtgRef3d': // TODO: lots of stuff
          ...
          stack.push(supbooks[1][ixti + 1] + "!" + utils.encode_cell(c));
          break;
        ...
        case 'PtgArea':
          ...
          stack.push(utils.encode_range(r));
          break;
          /* 2.5.198.28 */
        case 'PtgArea3d': // TODO: lots of stuff
          ...
          ixti = f[1][1];
          r = f[1][2];
          stack.push(supbooks[1][ixti + 1] + "!" + utils.encode_range(r));
          break;
          /* 2.5.198.41 */
        case 'PtgAttrSum':
        ...
        case 'PtgExp':
          ...
          if (supbooks.sharedf[utils.encode_cell(c)]) {
            var parsedf = (supbooks.sharedf[utils.encode_cell(c)]);
        ...
        case 'PtgMemArea':
          //stack.push("(" + f[2].map(utils.encode_range).join(",") + ")");
          break;
  ...
  function parse_ws_xml_dim(ws, s) {
    var d = utils.safe_decode_range(s);
    if (d.s.r <= d.e.r && d.s.c <= d.e.c && d.s.r >= 0 && d.s.c >= 0) ws["!ref"] = utils.encode_range(d);
  }
  ...
  function parse_ws_xml(data, opts, rels) {
    ...
    if (data.indexOf("</mergeCells>") !== -1) {
      ...
      for (ridx = 0; ridx != merges.length; ++ridx)
        mergecells[ridx] = utils.safe_decode_range(merges[ridx].substr(merges[ridx].indexOf("\"") + 1));
    }
    ...
    if (!s["!ref"] && refguess.e.c >= refguess.s.c && refguess.e.r >= refguess.s.r) s["!ref"] = utils.encode_range(refguess);
    if (opts.sheetRows > 0 && s["!ref"]) {
      var tmpref = utils.safe_decode_range(s["!ref"]);
      if (opts.sheetRows < +tmpref.e.r) {
        ...
        s["!fullref"] = s["!ref"];
        s["!ref"] = utils.encode_range(tmpref);
      }
    }
  ...
  function write_ws_xml_merges(merges) {
    ...
    for (var i = 0; i != merges.length; ++i) o += '<mergeCell ref="' + utils.encode_range(merges[i]) + '"/>';
    return o + '</mergeCells>';
  }
  ...
  function parse_ws_xml_hlinks(s, data, rels) {
    for (var i = 0; i != data.length; ++i) {
      ...
      var rng = utils.safe_decode_range(val.ref);
      for (var R = rng.s.r; R <= rng.e.r; ++R)
        for (var C = rng.s.c; C <= rng.e.c; ++C) {
          var addr = utils.encode_cell({
            c: C,
            r: R
          });
          if (!s[addr]) s[addr] = {
  ...
  function write_ws_xml_data(ws, opts, idx, wb) {
    var o = [],
      r = [],
      range = utils.safe_decode_range(ws['!ref']),
      cell, ref, rr = "",
      cols = [],
      R, C;
    for (C = range.s.c; C <= range.e.c; ++C) cols[C] = utils.encode_col(C);
    for (R = range.s.r; R <= range.e.r; ++R) {
      r = [];
      rr = utils.encode_row(R);
  ...
  function parse_ws_bin(data, opts, rels) {
    ...
    recordhopper(data, function ws_parse(val, R) {
      ...
      switch (R.n) {
        ...
        case 'BrtCellSt':
          ...
          s[utils.encode_col(C = val[0].c) + rr] = p;
    ...
    if (!s["!ref"] && (refguess.s.r < 1000000 || ref.e.r > 0 || ref.e.c > 0 || ref.s.r > 0 || ref.s.c > 0)) s["!ref"] = utils.encode_range(ref);
  ...
  function write_CELLTABLE(ba, ws, idx, opts, wb) {
    var range = utils.safe_decode_range(ws['!ref'] || "A1"),
      ref, rr = "",
      cols = [];
    ...
    for (var R = range.s.r; R <= range.e.r; ++R) {
      ...
      for (var C = range.s.c; C <= range.e.c; ++C) {
        /* *16384CELL */
        if (R === range.s.r) cols[C] = utils.encode_col(C);
        ref = cols[C] + rr;
        if (!ws[ref]) continue;
        /* write cell */
  ...
  function write_ws_bin(idx, opts, wb) {
    ...
    var s = wb.SheetNames[idx],
      ws = wb.Sheets[s] || {};
    var r = utils.safe_decode_range(ws['!ref'] || "A1");
  ...
  */
    ...
    while ((Rn = xlmlregex.exec(str))) switch (Rn[3]) {
      ...
      case 'Cell':
        if (Rn[1] === '/') {
          ...
          if ((!opts.sheetRows || opts.sheetRows > r) && cell.v !== undefined) cursheet[utils.encode_col(c) + utils.encode_row(r)] = cell;
          if (cell.HRef) {
            cell.l = {
              Target: cell.HRef,
      ...
      case 'Worksheet':
        ...
        if (Rn[1] === '/') {
          ...
          if (refguess.s.r <= refguess.e.r && refguess.s.c <= refguess.e.c) cursheet["!ref"] = utils.encode_range(refguess);
  ...
  function parse_workbook(blob, options) {
    ...
    var addcell = function addcell(cell, line, options) {
      ...
      lastcell = cell;
      last_cell = utils.encode_cell(cell);
      if (range.s) {
          ...
          case 'EOF':
            {
              ...
              if (range.e) {
                ...
                if (range.e.r > 0 && range.e.c > 0) {
                  range.e.r--;
                  range.e.c--;
                  out["!ref"] = utils.encode_range(range);
                  range.e.r++;
                  range.e.c++;
                }
          ...
          case 'ShrFmla':
            {
              ...
              /* TODO: capture range */
              shared_formulae[utils.encode_cell(last_formula.cell)] = val[0];
            }
            break;
          case 'LabelSst':
          ...
          case 'HLink':
            {
              for (rngR = val[0].s.r; rngR <= val[0].e.r; ++rngR)
                for (rngC = val[0].s.c; rngC <= val[0].e.c; ++rngC)
                  if (out[utils.encode_cell({
                      c: rngC,
                      r: rngR
                    })])
                    out[utils.encode_cell({
                      c: rngC,
                      r: rngR
                    })].l = val[1];
            }
          ...
          case 'Note':
            {
              ...
              cc = out[utils.encode_cell(val[0])];
